...
public class CameraSource {
    ...
    private Map<Detector,FrameProcessingRunnable> mFrameProcessor = new HashMap<>();
    ...
    public static class Builder {
        private final List<Detector<?>> mDetector;
         ...
         */
        public Builder(Context context, List<Detector<?>> detector) {
            if (context == null) {
        ...
        public CameraSource build() {
            for(Detector detector : mDetector) {
                mCameraSource.mFrameProcessor.put(detector, mCameraSource.new FrameProcessingRunnable(detector));
            }
    ...
    public void release() {
        synchronized (mCameraLock) {
            stop();
            for(FrameProcessingRunnable r : mFrameProcessor.values()) {
                r.release();
            }
        }
    }
    ...
    private void setActive(boolean active) {
        for(FrameProcessingRunnable mFrameProcessor : mFrameProcessor.values()) {
    ...
    private class CameraPreviewCallback implements Camera.PreviewCallback {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            setNextFrame(data, camera);
        }
    }

    private void setNextFrame(byte[] data, Camera camera) {
        for(FrameProcessingRunnable r : mFrameProcessor.values()) {
            r.setNextFrame(data, camera);
        }
    }
