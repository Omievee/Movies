...
private constructor() {
    ...
    private val mFrameProcessor = HashMap<Detector<*>, FrameProcessingRunnable>()
     ...
     */
    (context: Context?, private val mDetector: List<Detector<*>>?) {
        ...
        fun build(): CameraSource {
            if (mDetector != null) {
                for (detector in mDetector) {
                    mCameraSource.mFrameProcessor[detector] = mCameraSource.FrameProcessingRunnable(detector)
                }
            }
    ...
    fun release() {
        synchronized(mCameraLock) {
            stop()
            for (r in mFrameProcessor.values) {
                r.release()
            }
        }
    }
    ...
    fun start(): CameraSource {
        synchronized(mCameraLock) {
            ...
            mProcessingThread = object : Thread() {
                override fun run() {
                    for (r in mFrameProcessor.values) {
                        r.run()
                    }
    ...
    private fun setActive(active: Boolean) {
        for (mFrameProcessor in mFrameProcessor.values) {
    ...
    private fun setNextFrame(data: ByteArray, camera: Camera) {
        for (r in mFrameProcessor.values) {
